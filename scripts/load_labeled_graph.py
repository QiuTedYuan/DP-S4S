...
parser.add_argument('-p', '--path', required=True, type=str, help='Path to the graph, expect Path.tsv file.')
...
cursor.execute('''CREATE TABLE edge (FROM_ID INTEGER NOT NULL, TO_ID INTEGER NOT NULL, LABEL DATE NOT NULL)''')

cursor.execute('''CREATE INDEX on EDGE using hash (FROM_ID)''')
cursor.execute('''CREATE INDEX on EDGE using hash (TO_ID)''')

with open(args.path + ".tsv") as edges:
